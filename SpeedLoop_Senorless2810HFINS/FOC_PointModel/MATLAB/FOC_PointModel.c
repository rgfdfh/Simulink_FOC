 ...
 * C/C++ source code generated on : Tue Feb  3 11:42:53 2026
 *
...
void FOC_PointModel_step(void)
{
  ...
  if (rtb_ShiftArithmetic_f < 0) {
    rtb_ShiftArithmetic9_e1 = -rtb_ShiftArithmetic_f;
  } else {
    rtb_ShiftArithmetic9_e1 = rtb_ShiftArithmetic_f;
  }
  ...
  if (rtb_ShiftArithmetic1_o < 0) {
    rtb_ShiftArithmetic3 = -rtb_ShiftArithmetic1_o;
  } else {
    rtb_ShiftArithmetic3 = rtb_ShiftArithmetic1_o;
  }
  ...
  /* MinMax: '<S5>/Max' */
  if (rtb_ShiftArithmetic9_e1 >= rtb_ShiftArithmetic3) {
    rtb_ShiftArithmetic3 = rtb_ShiftArithmetic9_e1;
  }

  /* Abs: '<S5>/Abs3' */
  if (rtb_ShiftArithmetic2 < 0) {
    rtb_ShiftArithmetic9_e1 = -rtb_ShiftArithmetic2;
  } else {
    rtb_ShiftArithmetic9_e1 = rtb_ShiftArithmetic2;
  }
  ...
  /* MinMax: '<S5>/Max' */
  if (rtb_ShiftArithmetic3 >= rtb_ShiftArithmetic9_e1) {
    rtb_ShiftArithmetic9_e1 = rtb_ShiftArithmetic3;
  }
  ...
  if (rtDW.is_active_c2_FOC_PointModel == 0U) {
    ...
    switch (rtDW.is_c2_FOC_PointModel) {
     ...
     case IN_OpenStage:
      if (rtDW.temporalCounter_i1 >= 12500U) {
        guard1 = true;
   ...
   */
  rtb_Bias_o = (int16_T)((int16_T)(((int16_T)UDC) - 2) - (int16_T)((((int16_T)
  ...
  switch (rtAction) {
   ...
   case 1:
     ...
     *  Gain: '<S20>/Gain'
     */
    rtDW.Merge2 = rtDW.Open_Iq_Point;
   ...
   case 4:
     ...
     *  Switch: '<S58>/Switch'
     */
    if (rtb_ShiftArithmetic3 > SPD_STEP) {
      rtb_ShiftArithmetic3 = SPD_STEP;
    } else if (rtb_ShiftArithmetic3 < -SPD_STEP) {
    ...
    /* ArithShift: '<S19>/Shift Arithmetic' incorporates:
    ...
    /* Sum: '<S19>/Sum2' */
    rtb_FixPtSwitch_j = (int16_T)((int16_T)rtb_ShiftArithmetic3 - (int16_T)
      rtb_ShiftArithmetic9_e1);

    /* Sum: '<S64>/Sum' incorporates:
     *  ArithShift: '<S64>/Shift Arithmetic'
     *  ArithShift: '<S64>/这个是缩小周期增益的'
     *  Constant: '<S64>/周期定点化适当放大2^n'
     ...
     *  UnitDelay: '<S64>/Unit Delay'
     */
    rtb_Sum_ci = ((((rtDW.Ts_Point * rtb_FixPtSwitch_j) >> 15) *
     ...
     *  Switch: '<S65>/Switch'
     */
    if (rtb_Sum_ci > rtDW.Speed_Limit) {
      rtb_Sum_ci = rtDW.Speed_Limit;
    } else if (rtb_Sum_ci < -1) {
      /* Switch: '<S65>/Switch' */
      rtb_Sum_ci = -1;
    }
     ...
     */
    rtb_Min = rtb_FixPtSwitch_j * rtDW.Speed_Kp_Point + rtb_Sum_ci;
     ...
     *  Switch: '<S63>/Switch'
     */
    if (rtb_Min > rtDW.Speed_Limit) {
      rtb_Min = rtDW.Speed_Limit;
    } else if (rtb_Min < -rtDW.Speed_Limit) {
     ...
     *  Sum: '<S19>/Sum'
     */
    rtb_FixPtSwitch_j = (int16_T)(rtb_Min - rtb_Bias_o);
     ...
     */
    if (rtb_ShiftArithmetic9_e1 > 1500) {
      rtb_Sum12 = 0;
    } else {
      rtb_Sum12 = 648;
    }
     ...
     *  Sum: '<S19>/Sum6'
     *  Switch: '<S19>/Switch'
     *  Switch: '<S61>/FixPt Switch'
     */
    rtDW.Merge2 = (int16_T)((int16_T)(rtb_Sum12 - rtb_FixPtSwitch_j) -
      rtb_Sum_ix);
    ...
    rtDW.UnitDelay_DSTATE_g = rtb_Sum_ci;
   ...
   case 5:
     ...
     *  ArithShift: '<S55>/这个是缩小周期增益的'
     *  Constant: '<S55>/周期定点化适当放大2^n'
   ...
   *  Sum: '<S11>/Sum2'
   */
  rtb_Bias_o = rtConstP.pooled13[plook_u32u16_even0ckag((uint16_T)((uint32_T)
   ...
   *  Sum: '<S4>/Add2'
   */
  rtb_ShiftArithmetic3 = ((rtb_ShiftArithmetic1_o - rtb_ShiftArithmetic2) *
    18918) >> 15;
   ...
   *  Sum: '<S13>/Sum4'
   */
  rtb_Sum_ix = rtConstP.pooled13[plook_u32u16_even0ckag((uint16_T)((uint32_T)
  ...
  if (rtb_RelationalOperator) {
     ...
     *  Sum: '<S8>/Add2'
     */
    rtb_Min = rtb_FixPtSwitch_j - ((rtb_ShiftArithmetic_f * rtb_Bias_o +
      rtb_ShiftArithmetic3 * rtb_Sum_ix) >> 15);

    /* ArithShift: '<S44>/Shift Arithmetic' incorporates:
     *  ArithShift: '<S47>/Shift Arithmetic'
     *  Constant: '<S44>/周期定点化适当放大2^n'
     *  DataStoreRead: '<S10>/Data Store Read5'
     */
    rtb_Gain_av = rtDW.Curr_Ki_Point >> ((int16_T)TS_POINT_GAIN);
     ...
     */
    rtb_ShiftArithmetic9_e1 = ((rtDW.Ts_Point * rtb_Min) >> 15) * rtb_Gain_av +
     ...
     *  Switch: '<S45>/Switch'
     */
    if (rtb_ShiftArithmetic9_e1 > rtDW.Curr_Limit) {
      rtb_ShiftArithmetic9_e1 = rtDW.Curr_Limit;
    } else if (rtb_ShiftArithmetic9_e1 < -rtDW.Curr_Limit) {
     ...
     */
    rtb_Sum_ci = rtb_Min * rtDW.Curr_Kp_Point + rtb_ShiftArithmetic9_e1;
     ...
     *  Switch: '<S43>/Switch'
     */
    if (rtb_Sum_ci > rtDW.Curr_Limit) {
      rtb_Sum_ci = rtDW.Curr_Limit;
    } else if (rtb_Sum_ci < -rtDW.Curr_Limit) {
     ...
     *  Sum: '<S8>/Add1'
     */
    rtb_Min = rtb_Sum10_i - ((rtb_ShiftArithmetic3 * rtb_Bias_o -
     ...
     */
    rtb_Gain_av = ((rtDW.Ts_Point * rtb_Min) >> 15) * rtb_Gain_av +
     ...
     */
    rtb_Min = rtb_Min * rtDW.Curr_Kp_Point + rtb_Gain_av;
    ...
    /* Sum: '<S42>/Sum12' incorporates:
     *  Constant: '<S42>/Constant'
     *  DataTypeConversion: '<S42>/Data Type Conversion5'
     *  DataTypeConversion: '<S42>/Data Type Conversion6'
     *  Math: '<S42>/Square2'
     *  Math: '<S42>/Square3'
     *  Sqrt: '<S42>/Sqrt1'
     *  Sum: '<S42>/Sum11'
   ...
   *  Sum: '<S2>/Add'
   */
  rtb_ShiftArithmetic9_e1 = (rtDW.Ud * rtb_Bias_o - rtDW.Uq * rtb_Sum_ix) >> 15;
  ...
  if (rtb_RelationalOperator) {
     ...
     */
    rtb_Sum_ci = (((((rtDW.Obs_VtoI * rtb_ShiftArithmetic9_e1) >> 1) -
     ...
     */
    rtb_Gain_av = ((((rtb_Sum_ci - rtb_ShiftArithmetic_f) * rtDW.SMO_K -
                     rtDW.UnitDelay1_DSTATE) * rtDW.LPF_FRQ_Point) >> 4) +
     ...
     */
    rtb_Sum_ci = -((rtConstP.pooled13[plook_u32u16_even0ckag((uint16_T)
      ((uint32_T)(uint16_T)(rtY.Obs_Theta + 16512U) >> 7), 1U)] * rtb_Gain_av) >>
                   15) - ((rtConstP.pooled13[plook_u32u16_even0ckag((uint16_T)
      ((uint32_T)(uint16_T)(rtY.Obs_Theta + 128U) >> 7), 1U)] *
      rtb_ShiftArithmetic3) >> 15);
     ...
     */
    rtb_ShiftArithmetic3 = ((rtDW.Ts_Point * rtb_Sum_ci) >> 15) * (rtDW.PLL_Ki >>
      ((int16_T)TS_POINT_GAIN)) + rtDW.UnitDelay_DSTATE_p;
     ...
     */
    rtb_Sum_ci = rtb_Sum_ci * rtDW.PLL_Kp + rtb_ShiftArithmetic3;
     ...
     *  Switch: '<S30>/Switch'
     */
    if (rtb_Sum_ci > rtDW.PLL_Limit) {
      rtb_Sum_ci = rtDW.PLL_Limit;
    } else if (rtb_Sum_ci < -rtDW.PLL_Limit) {
     ...
     */
    rtb_Gain_av = (((rtb_Sum_ci >> ((int16_T)TS_POINT_GAIN)) * rtDW.Ts_Point) >>
                   15) + rtDW.UnitDelay_DSTATE_m;

    /* ArithShift: '<S25>/Shift Arithmetic13' incorporates:
     *  Switch: '<S30>/Switch2'
     */
    rtDW.ShiftArithmetic13 = rtb_Sum_ci >> 16;
  ...
  /* Outputs for Atomic SubSystem: '<S7>/LPF' */
   ...
   *  UnitDelay: '<S35>/Unit Delay9'
   */
  rtb_ShiftArithmetic3 = ((16 - rtb_Bias_o) * (rtDW.ShiftArithmetic13 *
    rtDW.Hz_RPM) + rtDW.UnitDelay9_DSTATE * rtb_Bias_o) >> 4;

  /* Update for UnitDelay: '<S35>/Unit Delay9' */
  rtDW.UnitDelay9_DSTATE = rtb_ShiftArithmetic3;

  /* End of Outputs for SubSystem: '<S7>/LPF' */

  /* ArithShift: '<S36>/Shift Arithmetic' incorporates:
   *  Gain: '<S36>/Gain1'
   */
   ...
   */
  rtb_Sum_ci = rtb_ShiftArithmetic9_e1 >> 1;
  ...
  /* SignalConversion generated from: '<S9>/Sum' incorporates:
   *  Sum: '<S36>/Sum'
   *  Sum: '<S36>/Sum1'
   */
  rtb_Gain_av = rtb_ShiftArithmetic9_e1;
  rtb_Gain = rtb_Min - rtb_Sum_ci;
  rtb_Sum_ci = -rtb_Min - rtb_Sum_ci;

  /* MinMax: '<S37>/Min' incorporates:
   *  SignalConversion generated from: '<S9>/Sum'
   */
  if (rtb_ShiftArithmetic9_e1 > rtb_Gain) {
    rtb_ShiftArithmetic9_e1 = rtb_Gain;
  }

  /* MinMax: '<S37>/Min1' incorporates:
   *  MinMax: '<S37>/Min'
   */
  if (rtb_Gain_av >= rtb_Gain) {
    rtb_Min = rtb_Gain_av;
  } else {
    rtb_Min = rtb_Gain;
  }
  ...
  /* MinMax: '<S37>/Min' */
  if (rtb_ShiftArithmetic9_e1 > rtb_Sum_ci) {
    rtb_ShiftArithmetic9_e1 = rtb_Sum_ci;
  }

  /* MinMax: '<S37>/Min1' incorporates:
   *  MinMax: '<S37>/Min'
   */
  if (rtb_Min < rtb_Sum_ci) {
    rtb_Min = rtb_Sum_ci;
  }
   ...
   *  Sum: '<S37>/Sum'
   */
  rtb_ShiftArithmetic9_e1 = -((rtb_ShiftArithmetic9_e1 + rtb_Min) >> 1);
  ...
  if (rtDW.Motor_State < 4) {
    if (!rtDW._MODE) {
      /* InitializeConditions for UnitDelay: '<S13>/Unit Delay7' */
      rtDW.UnitDelay7_DSTATE = 0U;

      /* InitializeConditions for UnitDelay: '<S13>/Unit Delay6' */
   ...
   */
  rtY.tABC[0] = (uint16_T)((int16_T)-((((rtb_Gain_av + rtb_ShiftArithmetic9_e1)
    - 51 * rtb_ShiftArithmetic_f) * rtDW.PWM_TH) >> 15) + rtb_Bias_o);
   ...
   *  UnaryMinus: '<S9>/Unary Minus'
   */
  rtY.tABC[1] = (uint16_T)((int16_T)-((((rtb_Gain + rtb_ShiftArithmetic9_e1) -
    51 * rtb_ShiftArithmetic_f) * rtDW.PWM_TH) >> 15) + rtb_Bias_o);
   ...
   *  UnaryMinus: '<S9>/Unary Minus'
   */
  rtY.tABC[2] = (uint16_T)((int16_T)-((((rtb_Sum_ci + rtb_ShiftArithmetic9_e1) -
    51 * rtb_ShiftArithmetic_f) * rtDW.PWM_TH) >> 15) + rtb_Bias_o);

  /* ArithShift: '<S7>/Shift Arithmetic4' incorporates:
   *  ArithShift: '<S35>/Shift Arithmetic9'
   */
  rtb_ShiftArithmetic1_o = rtb_ShiftArithmetic3 >> 8;
  ...
  if (rtb_ShiftArithmetic1_o < 0) {
     ...
     */
    rtY.Obs_Spd = (uint32_T)-rtb_ShiftArithmetic1_o;
  } else {
    /* Outport: '<Root>/Obs_Spd' incorporates:
     *  DataTypeConversion: '<S7>/Data Type Conversion'
     */
