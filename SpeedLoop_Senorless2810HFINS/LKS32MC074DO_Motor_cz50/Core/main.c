...
//	HAL_GPIO_Init();
//	HAL_Timer_Init();
	Dshot_Type = Dshot_Detect_Type_By_Level(); // 检测 DShot 类型
	Dshot_Init_Capture_By_Type(Dshot_Type); // 根据类型初始化定时器和 DMA
	HAL_MCPWM_Init();
	HAL_ADC_Init();
	HAL_OPA_Init();
	...
	{
		iuzero += (int16_T)(ADC0->DAT0); // A，直接强制int16
		iwzero += (int16_T)(ADC0->DAT1); // C，直接强制int16
	}
	...
	delay_ms(150);
	check_data[0] = (int16_T)(ADC0->DAT1) - (int16_T)iwzero;

	Self_Check(2, CHECK_FRQ1, CHECK_DUTY); /*BHCL*/
	delay_ms(150);
	check_data[1] = (int16_T)(ADC0->DAT1) - (int16_T)iwzero;

	Self_Check(3, CHECK_FRQ1, CHECK_DUTY); /*BHAL*/
	delay_ms(150);
	check_data[2] = (int16_T)(ADC0->DAT0) - (int16_T)iuzero;
...
//	NVIC_SetPriority(SysTick_IRQn, 2); // SysTick中断优先级
	NVIC_SetPriority(ADC0_IRQn, 2);	   // ADC中断优先级
...
//			NVIC_EnableIRQ(ADC0_IRQn);	  // 使能ADC中断
//		}
//		Dshot_Handle.Dshot_Decode_Flag =1;
//		Dshot_Type = DSHOT_TYPE_BIDIR;
		Dshot_Decode(&Dshot_Handle, &Motor_State, &Slow_State, &Speed_Ref);
		if(Dshot_Type == DSHOT_TYPE_BIDIR)
		{
			Dshot_Bidir_Encode_Telemetry(rtY.Obs_Spd, MOTOR_PN, &Dshot_Bidir_Frame);
		}
	}
}
/**
...
int32_T time;
uint32_T tmp;
__attribute__((section(".ramfunc")))//把adc中断也塞进ram
...
//		GPIO_SetBits(GPIO0, GPIO_Pin_5); // 进入adc中断指示
		...
		{
			PWMOutputs(DISABLE);
			vdczero += (int16_T)(ADC1->DAT0);
			iuzero += (int16_T)(ADC0->DAT0);
			iwzero += (int16_T)(ADC0->DAT1);
			zero_cnt++;
		}
		...
		if (zero_cnt > 64)
		{
			vdc = (int32_T)(int16_T)(ADC1->DAT0);
			iu = (int32_T)(int16_T)(ADC0->DAT0) - iuzero; // 哪个相对应哪个adc的DAT在adcgpio初始化中确定
			iw = (int32_T)(int16_T)(ADC0->DAT1) - iwzero; // 返回值是s16，有符号的
...
{
    if (Dshot_Handle.Telem_Tx_Busy == 0) // 作为接收模式才判断
    {
        Dshot_Check(&Dshot_Handle, &Motor_State, &Slow_State, &Speed_Ref); // 检测dshot信号
    }
    /*当接收标志位高、双向dshot类型、已经返回接收模式时，切换为输出模式*/
	if (Dshot_Handle.Data_Receive_Flag == 1 && Dshot_Type == DSHOT_TYPE_BIDIR && Dshot_Handle.Telem_Tx_Busy == 0 && Dshot_Handle.Decode_Lock == 0) // DSHOT_TYPE_BIDIR  DSHOT_TYPE_NORMAL
    {
        Dshot_Handle.Telem_Tx_Busy = 1;
        Dshot_Handle.Data_Receive_Flag = 0;
        Dshot_Init_Transmit_By_Frame(); // 更改timer为发射模式
    }
    /* 遥测发包结束检测：DMA_CHN0 计数为0则认为发送完成，切回接收模式 */
    if (Dshot_Handle.Telem_Tx_Busy == 1 && DMA_CHN0->CTMS == 0)
    {
        Dshot_Exit_Transmit_Mode(); // 配置回接收模式
        Dshot_Handle.Telem_Tx_Busy = 0;
    }
