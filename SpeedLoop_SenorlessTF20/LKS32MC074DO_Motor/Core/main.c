...
//	HAL_GPIO_Init();
	...
	for (int8_T i = 0; i < 3; i++)
	{
		if (check_data[i] < 10)
		{
			...
			while (1)
			{
				...
				Self_Check(2, CHECK_C, CHECK_DUTY1*2); /*BHCL*/
				delay_ms(300);
				PWMOutputs(DISABLE);delay_ms(100);
	...
	//SEGGER_RTT_ConfigUpBuffer(1,"JScope_i2i2i2",rtt_buffer,256,SEGGER_RTT_MODE_NO_BLOCK_SKIP);
