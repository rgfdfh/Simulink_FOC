...
void HAL_ADC0_Init(void)
{
    ...
    ADC_InitStruct.S1 = 3;                    // 第一段采样次数
    ADC_InitStruct.S2 = 0;                    // 第二段采样次数

    ADC_ClearIRQFlag(ADC0, ADC_ALL_IF);
    ADC_Init(ADC0, &ADC_InitStruct);

    ADC_CHN_GAIN_CFG(ADC0, CHN0, ADC_CHANNEL_3, ADC_GAIN3V6); // CH3是OPA2, W相A
    ADC_CHN_GAIN_CFG(ADC0, CHN1, ADC_CHANNEL_2, ADC_GAIN3V6); // CH2是OPA3, U相C
